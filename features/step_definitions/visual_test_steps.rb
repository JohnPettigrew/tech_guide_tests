...
When(/^I run TestEvolve\.mobile_app\.visual_check for label '(.*)' when no visual regressions have occurred$/) do |label|
...
When(/^I run TestEvolve\.mobile_app\.visual_check for label '(.*)' when visual regressions have occurred$/) do |label|
...
When(/^I run TestEvolve\.mobile_app\.visual_check for a label that has no baseline$/) do
  baseline_image = 'visual_testing/baseline/new-label.png'
  File.delete(baseline_image) if File.exist?(baseline_image)
  TestEvolve.mobile_app.visual_check('new-label')
end
...
And(/^a mobile_app baseline image exists for label '(.*)'$/) do |label|
  baseline_image = "visual_testing/baseline/#{label}.png"
  File.delete(baseline_image) if File.exist?(baseline_image)
  TestEvolve.mobile_app.visual_check(label)
end
...
When(/^multiple mobile app visual checks occur for different labels where no visual regressions have occurred$/) do
  TestEvolve.mobile_app.visual_check('label1')
  TestEvolve.mobile_app.visual_check('label2')
  TestEvolve.mobile_app.visual_check('label3')
  TestEvolve.mobile_app.visual_check('label4')
end
